...
export default class suspendItem extends ComponentBase {
    ...
    static createSuspendItem(res,e){
        let suspendItem: cc.Node;
        cc.loader.loadRes("./prefebs/suspend_prefabs/"+res,cc.Prefab,(err,pf)=>{
            if(!pf){
                console.debug("预制体为空");
            }
            ...
            suspendItem.setPosition(e.getLocation());
            suspendItem.on(cc.Node.EventType.MOUSE_MOVE,(event)=>{
